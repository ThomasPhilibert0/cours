...
def chaleur(N,dt,T):
    ...
    I = np.identity(taille)


    U0 = np.zeros((N+1)*(N+1))   #matrice de la sol qui commence avec condition initial u(t=0,x)=u0 on choisit ici u0 = 1
    A = matrix_lap(N)
    Un = np.zeros((N+1)*(N+1))
    
    for i in np.arange(1,dt,1+T):
        B = I - i*A
        Un = sci.spsolve(B,U0)
        U0 = Un
